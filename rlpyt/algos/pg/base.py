...
OptInfoOC = namedtuple("OptInfo", ["loss", "gradNorm", "entropy", "perplexity"])
...
class OCAlgo(RlAlgorithm):
    ...
    def process_returns(self, samples):
        ...
        normalize advantages.
        """
        reward, done, q, o, prev_o, pi_omega, bv = (samples.env.reward, samples.env.done,
            samples.agent.agent_info.q, samples.agent.agent_info.current_o, samples.agent.agent_info.prev_o, samples.agent.agent_info.inter_option_dist_info, samples.agent.bootstrap_value)
        done = done.type(reward.dtype)
