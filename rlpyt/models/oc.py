...
class OptionCriticHead_IndependentPreprocessor(Module):
    ...
    def __init__(self,
        ...
        self.pi = Sequential(pi_proc, pi_class(input_size, num_options, output_size, ortho_init=orthogonal_init, ortho_init_value=orthogonal_init_pol))
        if orthogonal_init:
            self.beta = Sequential(beta_proc, layer_init(Linear(input_size, num_options), orthogonal_init_base), nn.Sigmoid())
            self.q = Sequential(q_proc, layer_init(Linear(input_size, num_options), orthogonal_init_base))
            self.q_ent = Sequential(q_proc, layer_init(Linear(input_size, num_options),
                                    orthogonal_init_base)) if use_diversity else Dummy(num_options, out_value=0.)
            self.pi_omega = Sequential(pi_omega_proc, layer_init(Linear(input_size, num_options), orthogonal_init_pol),
                                          nn.Softmax(-1))
            self.interest = Sequential(int_proc, layer_init(Linear(input_size, num_options), orthogonal_init_base),
                                          nn.Sigmoid()) if use_interest else Dummy(num_options)
        else:
            self.beta = Sequential(beta_proc, Linear(input_size, num_options), nn.Sigmoid())
            self.q = Sequential(q_proc, Linear(input_size, num_options))
            self.q_ent = Sequential(q_proc, Linear(input_size, num_options)) if use_diversity else Dummy(num_options, out_value=0.)
            self.pi_omega = Sequential(pi_omega_proc, Linear(input_size, num_options), nn.Softmax(-1))
            self.interest = Sequential(int_proc, Linear(input_size, num_options), nn.Sigmoid()) if use_interest else Dummy(
                num_options)
