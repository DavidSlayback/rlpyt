...
class RunningMeanStdModel(torch.nn.Module):
    ...
    def forward(self, x, center=True):
        ...
        return torch.clamp((x - self.mean * float(center)) / obs_var.sqrt(), min=self.min_clip, max=self.max_clip)
...
class RunningReward(torch.nn.Module):
    ...
    # Convenience call to update and compute normalization in one pass. Set center to 1 for subtracting mean
    def forward(self, x, center=True):
        self.update(x)
        obs_var = torch.clamp(self.var, min=self.min_var)
        return torch.clamp((x - self.mean * float(center)) / obs_var.sqrt(), min=self.min_clip, max=self.max_clip)
