...
DistInfoM = namedarraytuple("DistInfo", ["mean"])
DistInfoStdM = namedarraytuple("DistInfoStd", ["mean", "log_std"])
...
class GaussianMixture(Distribution):
    ...
    With the additional wrinkle of maintaining over 2 dimensions
    """
    ...
    def kl(self, old_dist_info, new_dist_info, sampled_option):
        if self.squash is not None:
            raise NotImplementedError
        old_mean = old_dist_info.mean
        new_mean = new_dist_info.mean
    ...
    def mean_kl(self, old_dist_info, new_dist_info, sampled_option, valid=None):
        return valid_mean(self.kl(old_dist_info, new_dist_info, sampled_option), valid)
